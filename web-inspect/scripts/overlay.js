...
(() => {
  ...
  if (!TOKEN) { console.warn('[web-inspect] no token in script src'); return; }
  ...
  function pushConsole(level, args, explicitStack) {
    try {
      ...
      if (!stack && level === 'error') {
        try { stack = (new Error()).stack || null; } catch { /* ignore */ }
      }
      consoleBuffer.push({
        level,
    ...
    } catch { /* never let our hook break the page */ }
  }

  const _origError = console.error;
  console.error = function (...args) { pushConsole('error', args); return _origError.apply(this, args); };
  const _origWarn = console.warn;
  console.warn = function (...args) { pushConsole('warn', args); return _origWarn.apply(this, args); };
  window.addEventListener('error', (e) => {
    if (e.error instanceof Error) return pushConsole('error', [e.error]);
    pushConsole('error', [e.message, e.filename + ':' + e.lineno], e.error && e.error.stack);
  });
  ...
  window.fetch = async function (input, init) {
    ...
    try {
      ...
      if (!res.ok) {
        networkBuffer.push({ url, method, status: res.status, duration: Math.round(performance.now() - start), ts: Date.now() });
        if (networkBuffer.length > MAX_NETWORK_BUFFER) networkBuffer.shift();
      }
      return res;
    } catch (err) {
      networkBuffer.push({ url, method, status: 0, error: String(err), duration: Math.round(performance.now() - start), ts: Date.now() });
      if (networkBuffer.length > MAX_NETWORK_BUFFER) networkBuffer.shift();
      throw err;
    }
  };
  ...
  XMLHttpRequest.prototype.send = function () {
    this.addEventListener('loadend', () => {
      if (this.status >= 400 || this.status === 0) {
        networkBuffer.push({
          url: this.__wiUrl, method: this.__wiMethod, status: this.status,
          duration: Math.round(performance.now() - (this.__wiStart || performance.now())),
          ts: Date.now(),
        });
        if (networkBuffer.length > MAX_NETWORK_BUFFER) networkBuffer.shift();
      }
  ...
  function parseSourceMap(rawJson) {
    let m;
    try { m = JSON.parse(rawJson); } catch { return null; }
    if (!m || typeof m.mappings !== 'string' || !Array.isArray(m.sources)) return null;
    const segments = [];
    ...
    for (let genLine = 0; genLine < lines.length; genLine++) {
      ...
      for (const segStr of lineStr.split(',')) {
        ...
        if (v.length >= 4) {
          ...
          segments.push({ genLine, genCol, srcIdx, origLine, origCol, nameIdx: v.length >= 5 ? nameIdx : -1 });
        }
      }
    }
    return { sources: m.sources, sourceRoot: m.sourceRoot || '', names: m.names || [], segments };
  }
      ...
      try {
        ...
        if (!mapText) {
          ...
          if (ref.startsWith('data:')) {
            ...
            if (!dm) return null;
            try { mapText = atob(dm[1]); } catch { return null; }
          } else {
            try {
    ...
    for (const rule of Array.from(rules)) {
      if (rule.type === 1 && rule.selectorText) {
        ...
        for (const sel of sels) {
          try {
            if (el.matches(sel)) { out.push({ selectorText: sel, href }); break; }
          } catch { /* invalid for matches() (e.g. ::-webkit-*) */ }
        }
        ...
        try {
          if (rule.type === 4 && rule.media && rule.media.mediaText
              && !window.matchMedia(rule.media.mediaText).matches) continue;
  ...
  async function captureCssSources(el) {
    const matches = [];
    let sheets;
    try { sheets = Array.from(document.styleSheets); } catch { return null; }
    ...
    for (const m of top) {
      try {
        if (!m.href) {
          out.push({ source: null, line: null, column: null, selectorText: m.selectorText });
          continue;
        }
        ...
        const [text, map] = await Promise.all([fetchText(absUrl), fetchMap(absUrl)]);
        let source = null, line = null, column = null;
        if (text && map) {
          const pos = locateRuleOffset(text, m.selectorText);
          if (pos) {
            const orig = originalPositionFor(map, pos.line, pos.column);
            if (orig) { source = orig.source; line = orig.line; column = orig.column; }
          }
        }
  ...
  function parseStackFrames(stack) {
    ...
    for (const raw of stack.split('\n')) {
      ...
      if (m) {
        frames.push({ name: m[1] || '', url: m[2], line: +m[3], column: +m[4], raw: ln });
        continue;
      }
  ...
  async function resolveStack(stack) {
    ...
    for (const f of frames) {
      let source = null, line = null, column = null, name = f.name || null;
      try {
        const abs = new URL(f.url, location.href);
        if (abs.origin === location.origin && /\.m?js(\?|$)/.test(abs.pathname)) {
          const map = await fetchMap(abs.toString());
          if (map) {
            const pos = originalPositionFor(map, f.line - 1, f.column - 1);
            if (pos) { source = pos.source; line = pos.line; column = pos.column; name = pos.name || name; }
          }
        }
      } catch { /* ignore */ }
  ...
  async function resolveConsoleErrors(errors) {
    ...
    for (const e of errors) {
      if (!e.stack) { out.push(e); continue; }
  ...
  function loadQueue() {
    try {
      ...
      if (typeof data.sessionDescription === 'string') sessionDescription = data.sessionDescription;
    } catch { /* ignore */ }
  }
  function saveQueue() {
    try { localStorage.setItem(QUEUE_KEY, JSON.stringify({ queue, sessionDescription })); } catch { /* ignore */ }
  }
  ...
  function buildSelector(el) {
    ...
    while (cur && cur !== document.body && parts.length < 5) {
      ...
      if (cur.className && typeof cur.className === 'string') {
        const cls = cur.className.trim().split(/\s+/).filter((c) => c && !c.startsWith('web-inspect-')).slice(0, 2).map((c) => `.${CSS.escape(c)}`).join('');
        part += cls;
      }
  ...
  function getInterestingStyles(el) {
    ...
    const keys = ['display', 'position', 'width', 'height', 'padding', 'margin', 'border',
      'color', 'backgroundColor', 'fontSize', 'fontWeight', 'fontFamily',
      'flexDirection', 'gridTemplateColumns', 'overflow', 'visibility', 'opacity',
      'transform', 'zIndex'];
    const out = {};
  ...
  function snapshotElement(el) {
    ...
    return {
      ...
      id: el.id || '',
      classes: (typeof el.className === 'string' ? el.className.split(/\s+/) : []).filter((c) => c && !c.startsWith('web-inspect-')),
  ...
  function loadScreenshotLib() {
    ...
    screenshotLibPromise = new Promise((resolve) => {
      ...
      s.onload = () => {
        screenshotLib = window.modernScreenshot || window.ModernScreenshot || null;
  ...
  async function captureElement(el) {
    try {
      ...
      const png = await lib.domToPng(el, { backgroundColor: '#fff', scale: 1, timeout: 4000 });
  ...
  function renderList() {
    ...
    els.list.innerHTML = queue.map((a, i) => `
  ...
  function isOurs(el) {
    while (el && el !== document) {
      if (el.nodeType === 1 && (el.hasAttribute && (el.hasAttribute(ROOT_ATTR) || el.hasAttribute(IGNORE_ATTR)))) return true;
      el = el.parentNode;
    }
    return false;
  }
  ...
  function openCommentPopover(targetEl, x, y) {
    ...
    const px = Math.min(window.innerWidth - 300, Math.max(8, x ?? rect.x + rect.width / 2 - 140));
    const py = Math.min(window.innerHeight - 200, Math.max(8, (y ?? rect.y + rect.height) + 8));
    ...
    pop.addEventListener('click', async (e) => {
      ...
      if (act === 'save') {
        ...
        closePopover();
        if (!comment) { showToast('Comment empty — discarded'); return; }
  ...
  function openSidebar() { els.sidebar.classList.remove('collapsed'); }
  function closeSidebar() { els.sidebar.classList.add('collapsed'); }
  ...
  root.addEventListener('click', async (e) => {
    ...
    if (act === 'pick') startPicker();
      ...
      if (queue.length && !confirm(`Clear ${queue.length} annotation(s)?`)) return;
      queue = [];
      sessionDescription = '';
  ...
  async function sendBatch() {
    ...
    try {
      const res = await fetch(`${HELPER_ORIGIN}/batch?token=${encodeURIComponent(TOKEN)}`, {
        method: 'POST',
      ...
      for (const a of queue) {
        ...
        try {
          ...
          if (el) {
            ...
            a.element.boundingRect = { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) };
            dirty = true;
          }
        } catch { /* ignore */ }
      }
